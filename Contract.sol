...
contract TaskPOAP is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    string private _baseURI;

    mapping(uint256 => string) _tokenURI;
    mapping(uint256 => bool) _tokenTransferable;

    constructor() ERC1155("TaskPOAP") {}
    ...
    function setTransferable(uint256 _tokenId, bool _transferable) public onlyOwner {
        ...
        override(ERC1155, ERC1155Supply)
    {
        ...
        if (to != address(0) && from != address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                uint256 id = ids[i];
                require(_tokenTransferable[id], "ERC1155: token not allow to transfer");
            }
        }
