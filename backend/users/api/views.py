...
from poap.users.api.serializers import (
    WalletVerifySerializer, WallerUserSerializer, WalletUserCreateSerializer,
    AuthGithubSerializer, WalletGithubSerializer,
)
from poap.users.models import WalletNonce, WalletUser, WalletGithub
...
class AuthGithub(APIView):
    permission_classes = (IsAuthenticated,)

    def post(self, request):
        serializer = AuthGithubSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        WalletGithub.objects.filter(user=request.user).delete()
        github = WalletGithub.objects.create(
            rid=resp["id"],
            username=resp["login"],
            nickname=resp["name"],
            user=request.user,
        )
        return Response(status=status.HTTP_200_OK, data=WalletGithubSerializer(github).data)
