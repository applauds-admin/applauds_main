...
task("functions-request", "Initiates a request from a Functions client contract")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    console.log(ethers.utils.formatBytes32String("TEST"))
    const transactionEstimateGas = await clientContract.estimateGas.simpleExecuteRequest(
      requestConfig.source,
      // requestConfig.secrets && Object.keys(requestConfig.secrets).length > 0 ? simulatedSecretsURLBytes : [],
      subscriptionId,
      requestConfig.args ?? [],
    ...
    console.log("transactionEstimateGas:", transactionEstimateGas.mul("2"))
    const newGas = transactionEstimateGas.mul("2")

    await utils.promptTxCost(newGas, hre, true)
    ...
    await new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        requestTx = await clientContract.simpleExecuteRequest(
          request.source,
          // request.secrets ?? [],
          subscriptionId,
          request.args ?? [],
